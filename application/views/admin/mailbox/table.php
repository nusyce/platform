...
array_push($where, ' AND to_staff_id = '.get_user_id());
...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<div class="" style="    display: inline-table;"><input type="checkbox" value="' . $aRow['id'] . '"><label></label></div>
    ...
    $row[] = $content.'<span class="'.$read.'">'.$aRow['subject'].' - </span><span class="text-muted" style="display: inline-table;">'.clear_textarea_breaks(text_limiter($aRow['body'],8,'...')).'</span>'.$has_attachment.'</a>';
