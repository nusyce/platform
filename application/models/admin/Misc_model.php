...
class Misc_model extends CI_Model
{
    ...
    public function delete_reminder($id)
    {
        ...
        if ($reminder->creator == get_user_id() || is_admin()) {
            $this->db->where('id', $id);
            ...
            return false;
        } //$reminder->creator == get_user_id() || is_admin()
        return false;
    }
    ...
    public function perform_search($q)
    {
        ...
        if ($have_assigned_customers || $have_permission_customers_view) {
            ...
            if ($have_assigned_customers && !$have_permission_customers_view) {
                $this->db->where(db_prefix() . 'clients.userid IN (SELECT customer_id FROM ' . db_prefix() . 'customer_admins WHERE staff_id=' . get_user_id() . ')');
            }
        ...
        $where_contacts = '';
        if ($have_assigned_customers && !$have_permission_customers_view) {
            $where_contacts = db_prefix() . 'contacts.userid IN (SELECT customer_id FROM ' . db_prefix() . 'customer_admins WHERE staff_id=' . get_user_id() . ')';
        }
        ...
        if (!$is_admin) {
            if (!$tasks) {
                $where = '(id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid = ' . get_user_id() . ') OR id IN (SELECT taskid FROM ' . db_prefix() . 'task_followers WHERE staffid = ' . get_user_id() . ') OR (addedfrom=' . get_user_id() . ' AND is_added_from_contact=0) ';
                if (get_option('show_all_tasks_for_project_member') == 1) {
                    $where .= ' OR (rel_type="project" AND rel_id IN (SELECT project_id FROM ' . db_prefix() . 'project_members WHERE staff_id=' . get_user_id() . '))';
                }
    ...
    public function _search_leads($q, $limit = 0, $where = [])
    {
        ...
        if (is_staff_member()) {
            ...
            if (!$has_permission_view) {
                $this->db->where('(assigned = ' . get_user_id() . ' OR addedfrom = ' . get_user_id() . ' OR is_public=1)');
            }
        ...
        if (is_staff_member() || (!is_staff_member() && get_option('access_tickets_to_none_staff_members') == 1)) {
            ...
            if (!$is_admin && get_option('staff_access_only_assigned_departments') == 1) {
                ...
                if (count($departments_ids) > 0) {
                    $where = 'department IN (SELECT departmentid FROM ' . db_prefix() . 'staff_departments WHERE departmentid IN (' . implode(',', $departments_ids) . ') AND staffid="' . get_user_id() . '")';
                }
            }
    ...
    public function _search_contracts($q, $limit = 0)
    {
        ...
        if ($has_permission_view_contracts || has_permission('contracts', '', 'view_own')) {
            ...
            if (!$has_permission_view_contracts) {
                $this->db->where(db_prefix() . 'contracts.addedfrom', get_user_id());
            }
    ...
    public function _search_credit_notes($q, $limit = 0)
    {
        ...
        if ($has_permission_view_credit_notes || $has_permission_view_credit_notes_own) {
            ...
            if (!$has_permission_view_credit_notes) {
                $this->db->where(db_prefix() . 'creditnotes.addedfrom', get_user_id());
            }
    ...
    public function _search_expenses($q, $limit = 0)
    {
        ...
        if ($has_permission_expenses_view || $has_permission_expenses_view_own) {
            ...
            if (!$has_permission_expenses_view) {
                $this->db->where(db_prefix() . 'expenses.addedfrom', get_user_id());
            }
