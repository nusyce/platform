...
class Paypal extends MY_Controller
{
    ...
    function index(){
        $this->load->view('admin/paypal/payment_credit_form');
    }
...
// payment approval/ cancellation.
        ...
        $redirectUrls->setReturnUrl($baseUrl."admin/paypal/getPaymentStatus")
            ->setCancelUrl($baseUrl."admin/paypal/getPaymentStatus");
...
// the above types and intent set to sale 'sale'
		...
		$this->session->set_userdata('pack_data',$pack_data);
		/*$hash=md5($payment->getId());
		$_SESSION['paypal_hash'] = $hash;
		$store=$db->prepare("insert into mar_transtation_paypal (user_id,payment_id,hash,complete) values (:user_id, payment_id, :hash,0)");
		$store->execute(['user_id' => 1,
    ...
    public function getPaymentStatus()
    {
        ...
        if ($result->getState() == 'approved') {
            ...
            redirect('admin/paypal/success');
        }
        $this->session->set_flashdata('success_msg','Payment failed');
        redirect('admin/paypal/cancel');
    }
    function success(){
        $this->load->view("admin/paypal/success");
    }
    function cancel(){
        $this->load->view("admin/paypal/scancel");
    }

    function load_refund_form(){
        $this->load->view('admin/paypal/sRefund_payment_form');
    }
