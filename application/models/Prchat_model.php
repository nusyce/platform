...
class Prchat_model extends CI_Model
{
    /**
    ...
    {
        $data = [];
        //$user = get_staff(get_user_id());

        $this->db->select('admin_id as staffid, firstname, lastname, image, last_login,  admin_role_id, linkedin, skype, facebook');
       ...
       /* if ($user->role != 2) {
        ...
        $users = $this->db->get(db_prefix() . 'admin')->result_array();

        foreach ($users as $key => $user) {
            $sql = "SELECT message,sender_id,time_sent FROM " . db_prefix() . "chatmessages WHERE (sender_id = " . get_user_id() . " AND reciever_id = {$user['staffid']}) OR (sender_id = {$user['staffid']} AND reciever_id = " . get_user_id() . ") ORDER BY id DESC LIMIT 0, 1";
    ...
    {
        $CI = &get_instance();
        $CI->db->from(db_prefix() . 'admin');
        $CI->db->where('admin_id', $id);
        $data = $CI->db->get()->row('image');
    ...
    public function getGroupMessages($group_id, $limit, $offset)
    {
        if ($group_id !== null) {
            ...
            $this->db->join(TABLE_STAFF, '' . TABLE_STAFF . '.admin_id=' . TABLE_CHATGROUPMEMBERS . '.member_id');
    ...
    public function deleteMessage($id, $mixed_id)
    {
        ...
        if (strpos($mixed_id, 'group_id') !== false) {
            ...
            if (prchat_checkMessageIfFileExists($possible_file)) {
                ...
                $this->db->delete(db_prefix() . 'chatsharedfiles', array('sender_id' => get_user_id(), 'reciever_id' => $mixed_id, 'file_name' => $file_name));
                $this->db->delete(db_prefix() . 'chatsharedfiles', array('sender_id' => $mixed_id, 'reciever_id' => get_user_id(), 'file_name' => $file_name));
            }
            ...
            if (is_array($members) && !empty($members)) {
                ...
                foreach ($members as $member_id) {
                    ...
                    $pusher->trigger('presence-mychanel', 'send-event', array(
                        'message' => pr_chat_convertLinkImageToString($message, get_user_id(), $member_id),
                        'from' => get_user_id(),
                        'to' => $member_id,
                        'global' => true
                    ));
    ...
    public function getMyGroups()
    {
        ...
        foreach ($groups as $key => $group) {
            $groups[$key]['members'] = $this->db->query("SELECT member_id, firstname, lastname, group_id FROM " . TABLE_CHATGROUPMEMBERS . " JOIN " . TABLE_STAFF . " ON " . TABLE_STAFF . ".admin_id=" . TABLE_CHATGROUPMEMBERS . ".member_id WHERE group_id=" . $group['id'] . " AND member_id=" . $id . "")->result_array();
        }
