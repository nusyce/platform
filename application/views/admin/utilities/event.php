...
<div class="modal fade _event" id="viewEvent">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <?php echo form_open('admin/admin/personalplan',array('id'=>'calendar-event-form')); ?>
      <div class="modal-body">
        <div class="row">
          <div class="col-md-12">
            <?php if($event->userid != get_user_id()){ ?>
              <div class="alert alert-info"><?php echo _l('event_created_by','<a href="'.admin_url('profile/'.$event->userid).'" target="_blank">'.get_user_profile_image($event->userid)).'</a>'; ?></div>
            <?php } ?>
            <?php if(($event->userid == get_user_id() && has_permission('personalplan',get_user_id(),'edit')) || is_admin()) { ?>
              <a href="<?php echo admin_url('profile/'.$event->userid); ?>"><?php echo get_user_profile_image($event->userid,array('staff-profile-xs-image')); ?> <?php echo get_user_full_name($event->userid)  ?></a>
          <hr />

            <div class="form-group">
              <?php
                $staffs = $_SESSION['staff'];
                echo render_select('user[]', $staffs, array('admin_id', array('firstname', 'lastname')), 'Mitabeiter', '', array('required' => true, 'multiple' => true) );
              ?>
              </div>

              <!--?php echo form_hidden('user[]',implode($event->user); ?-->
              <?php echo form_hidden('eventid',$event->eventid); ?>
              <?php echo render_input('title',_l('utility_calendar_new_event_placeholder'),$event->title); ?>
              <?php echo render_textarea('description',_l('event_description'),$event->description,array('rows'=>5)); ?>
              <?php echo render_datetime_input('start',_l('utility_calendar_new_event_start_date'),_dt($event->start)); ?>
              <div class="clearfix mtop15"></div>
              <?php echo render_datetime_input('end',_l('utility_calendar_new_event_end_date'),_dt($event->end)); ?>
          ...
          <hr />
          <div class="checkbox checkbox-primary" style="padding: 0">
