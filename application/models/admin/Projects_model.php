...
class Projects_model extends CI_Model
{
    ...
    public function pin_action($id)
    {
        ...
        $this->db->where('staff_id', get_user_id());
    ...
    public function get($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($project) {
               /* $project->shared_vault_entries = $this->client_model->get_vault_entries($project->clientid, ['share_in_projects' => 1]);
    ...
    public function get_tasks($id, $where = [], $apply_restrictions = false, $count = false)
    {
        ...
        if (!is_client_logged_in() && is_staff_logged_in()) {
            $select .= ',(SELECT staffid FROM ' . db_prefix() . 'task_assigned WHERE taskid=' . db_prefix() . 'tasks.id AND staffid=' . get_user_id() . ') as current_user_is_assigned';
        }
        ...
        if ($apply_restrictions == true) {
            if (!is_client_logged_in() && !$has_permission && $show_all_tasks_for_project_member == 0) {
                $this->db->where('(
                    ' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE user_id=' . get_user_id() . ')
                    OR ' . db_prefix() . 'tasks.id IN(SELECT taskid FROM ' . db_prefix() . 'task_followers WHERE user_id=' . get_user_id() . ')
                    OR is_public = 1
                    OR (addedfrom =' . get_user_id() . ' AND is_added_from_contact = 0)
                    )');
            }
        }
        ...
        foreach ($type_data as $data) {
            if ($type == 'milestones') {
                ...
                if ($data['staff_id'] != 0) {
                    ...
                    $name = get_user_full_name($data['staff_id']);
                } else {
    ...
    public function get_all_projects_gantt_data($filters = [])
    {
        ...
        foreach ($statuses as $status) {
            ...
            if (!has_permission('projects', '', 'view')) {
                $this->db->where(db_prefix() . 'projects.id IN (SELECT project_id FROM ' . db_prefix() . 'project_members WHERE staff_id=' . get_user_id() . ')');
            }
    ...
    private function _notify_project_members_status_change($id, $old_status, $new_status)
    {
        ...
        foreach ($members as $member) {
            if ($member['staff_id'] != get_user_id()) {
                $notified = add_notification([
                    'fromuserid' => get_user_id(),
                ...
                if (isset($project_members)) {
                    if (!in_array($project_member['staff_id'], $project_members)) {
                        ...
                        if ($this->db->affected_rows() > 0) {
                            ...
                            $this->log_activity($id, 'project_activity_removed_team_member', get_user_full_name($project_member['staff_id']));
                            $affectedRows++;
                        }
                    }
                } else {
                ...
                foreach ($project_members as $staff_id) {
                    ...
                    if (!$_exists) {
                        ...
                        if ($this->db->affected_rows() > 0) {
                            ...
                            $this->log_activity($id, 'project_activity_added_team_member', get_user_full_name($staff_id));
                            $affectedRows++;
                        }
                    }
                }
    ...
    public function remove_team_member($project_id, $staff_id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->log_activity($project_id, 'project_activity_removed_team_member', get_user_full_name($staff_id));
    ...
    public function get_timesheets($project_id, $tasks_ids = [])
    {
        ...
        if (count($tasks_ids) > 0) {
            ...
            foreach ($timesheets as $t) {
                ...
                $timesheets[$i]['staff_name'] = get_user_full_name($t['staff_id']);
        ...
        if ($comment->contact_id != 0) {
            ...
            if (is_client_logged_in()) {
                ...
                if (is_staff_logged_in()) {
                    if ($comment->staff_id == get_user_id()) {
                        $comment->created_by_current_user = true;
                    } else {
                        $comment->created_by_current_user = false;
                    }
    ...
    public function add_discussion_comment($data, $discussion_id, $type)
    {
        ...
        if (is_client_logged_in()) {
            ...
            $_data['contact_id'] = 0;
            $_data['staff_id'] = get_user_id();
            $_data['fullname'] = get_user_full_name($_data['staff_id']);
        }
        ...
        if ($insert_id) {
            ...
            if (is_client_logged_in()) {
                ...
                $notification_data['fromuserid'] = get_user_id();
            }

            $members = $this->get_project_members($discussion->project_id);
            $notifiedUsers = [];
            foreach ($members as $member) {
                if ($member['staff_id'] == get_user_id() && !is_client_logged_in()) {
                    continue;
                }
    ...
    public function copy($project_id, $data)
    {
        ...
        $_new_data['addedfrom'] = get_user_id();
    ...
    public function get_activity($id = '', $limit = '', $only_project_members_activity = false)
    {
        if (!is_client_logged_in()) {
            ...
            if (!$has_permission) {
                $this->db->where('project_id IN (SELECT project_id FROM ' . db_prefix() . 'project_members WHERE staff_id=' . get_user_id() . ')');
            }
        }
        if (is_client_logged_in()) {
            $this->db->where('visible_to_customer', 1);
    ...
    public function log_activity($project_id, $description_key, $additional_data = '', $visible_to_customer = 1)
    {
        if (!DEFINED('CRON')) {
            if (is_client_logged_in()) {
                ...
                $data['contact_id'] = 0;
                $data['staff_id'] = get_user_id();
                $data['fullname'] = get_user_full_name(get_user_id());
            }
        } else {
            $data['contact_id'] = 0;
            $data['staff_id'] = 0;
    ...
    public function send_project_email_template($project_id, $staff_template, $customer_template, $action_visible_to_customer, $additional_data = [])
    {
        ...
        foreach ($members as $member) {
            if (is_staff_logged_in() && $member['staff_id'] == get_user_id()) {
                continue;
            }
