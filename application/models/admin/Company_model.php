...
class Company_model extends CI_Model
{
    ...
    {
        if (is_numeric($id)) {
            $this->db->where(db_prefix() . 'companies.id', $id);
            return $this->db->get(db_prefix() . 'companies')->row();
        } else {
            return $this->db->get(db_prefix() . 'companies')->result_array();
        }
    }
    ...
    public function add($data)
    {
        if ($data) {
			...
			$this->Activity_model->add_log('New company [ID: ' . $insert_id . ']');
            return $insert_id;
        }
