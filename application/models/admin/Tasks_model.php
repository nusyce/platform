...
class Tasks_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('admin/mieter_model');
    }
    ...
    public function get($id, $where = [])
    {
        ...
        if ($task) {
            ...
            $task->client_data = $this->client_model->get($task->clients);
            if (is_staff_logged_in()) {
                $task->current_user_is_assigned = $this->is_task_assignee(get_user_id(), $id);
                $task->current_user_is_creator = $this->is_task_creator(get_user_id(), $id);
            }
    ...
    public function add($data, $clientRequest = false)
    {
        ...
        if ($insert_id) {
			...
			$this->Activity_model->add_log('New Task Added [ID:' . $insert_id . ', Name: ' . $data['name'] . ']');
    ...
    public function duplicate($data, $clientRequest = false)
    {
        ...
        $data['dateadded'] = date('Y-m-d H:i:s');
        $data['addedfrom'] = $clientRequest == false ? get_user_id() : get_contact_user_id();
        $data['is_added_from_contact'] = $clientRequest == false ? 0 : 1;
    ...
    public function update($data, $id, $clientRequest = false)
    {
        ...
        if ($this->db->affected_rows() > 0) {
			...
			$this->Activity_model->add_log('Task Updated [ID:' . $id . ', Name: ' . $data['name'] . ']');
    ...
    public function add_task_followers($data)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $additional_notification_data = serialize([
                get_user_full_name($data['follower']),
                $taskName,
            ]);

            if ($data['follower'] == get_user_id()) {
                $additional_notification_data = serialize([
                    $taskName,
                ]);
    ...
    public function add_task_assignees($data, $cronOrIntegration = false, $clientRequest = false)
    {
        ...
        if ($cronOrIntegration) {
            ...
            $assignData['assigned_from'] = get_user_id();
        }
        ...
        if ($assigneeId) {
            ...
            if (get_user_id() != $data['assignee'] || $clientRequest) {
                $notification_data = [
            ...
            $additional_notification_data = serialize([
                get_user_full_name($data['assignee']),
                $task->name,
            ]);
            if ($data['assignee'] == get_user_id()) {
            ...
            if ($task->rel_type == 'project') {
                $this->projects_model->log_activity($task->rel_id, 'project_activity_new_task_assignee', $task->name . ' - ' . get_user_full_name($data['assignee']), $task->visible_to_client);
            }
    ...
    {
        $this->db->select('id,' . db_prefix() . 'task_followers.user_id as followerid, CONCAT(firstname, " ", lastname) as full_name');
        $this->db->from(db_prefix() . 'task_followers');
        $this->db->join(db_prefix() . 'admin', db_prefix() . 'admin.admin_id = ' . db_prefix() . 'task_followers.user_id');
        $this->db->where('taskid', $id);
    ...
    {
        $this->db->select('id,' . db_prefix() . 'task_assigned.user_id as assigneeid,assigned_from,firstname,lastname,CONCAT(firstname, " ", lastname) as full_name,is_assigned_from_contact');
        $this->db->from(db_prefix() . 'task_assigned');
        $this->db->join(db_prefix() . 'admin', db_prefix() . 'admin.admin_id = ' . db_prefix() . 'task_assigned.user_id');
        $this->db->where('taskid', $id);
    ...
    public function get_task_comments($id)
    {
        ...
        $this->db->select('id,dateadded,content,moment,' . db_prefix() . 'admin.firstname,' . db_prefix() . 'admin.lastname,' . db_prefix() . 'task_comments.user_id,' . db_prefix() . 'task_comments.contact_id as contact_id,file_id,CONCAT(firstname, " ", lastname) as staff_full_name');
        $this->db->from(db_prefix() . 'task_comments');
        $this->db->join(db_prefix() . 'admin', db_prefix() . 'admin.admin_id = ' . db_prefix() . 'task_comments.user_id', 'left');
        $this->db->where('taskid', $id);
    ...
    public function edit_comment($data)
    {
        ...
        $comment = $this->db->get(db_prefix() . 'task_comments')->row();
        if ($comment->staffid == get_user_id() || has_permission('tasks', '', 'edit') || $comment->contact_id == get_contact_user_id()) {
    ...
    public function remove_comment($id, $force = false)
    {
        ...
        if ($comment->staffid == get_user_id() || has_permission('tasks', '', 'delete') || $comment->contact_id == get_contact_user_id() || $force === true) {
            $comment_added = strtotime($comment->dateadded);
            $minus_1_hour = strtotime('-1 hours');
    ...
    public function remove_assignee($id, $taskid)
    {
        ...
        // Stop all timers
        $this->db->where('task_id', $taskid);
        $this->db->where('user_id', $assignee_data->admin_id);
        ...
        if ($this->db->affected_rows() > 0) {
            if ($task->rel_type == 'project') {
                //$this->projects_model->log_activity($task->rel_id, 'project_activity_task_assignee_removed', $task->name . ' - ' . get_user_full_name($assignee_data->staffid), $task->visible_to_client);
            }
    ...
    public function delete_task($id, $log_activity = true)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            /*if ($task->rel_type == 'project' && $log_activity == true) {
				$this->Activity_model->add_log('project_activity_task_deleted [Name: ' . $task->name. ']');
            }*/
    ...
    public function timer_tracking($task_id = '', $timer_id = '', $note = '', $adminStop = false)
    {
        ...
        if ($task_id !== '0' && $adminStop == false) {
            if (!$this->is_task_assignee(get_user_id(), $task_id)) {
                return false;
    ...
    public function timesheet($data)
    {
        ...
        $timesheet_staff_id = get_user_id();
    ...
    public function task_tracking_stats($id)
    {
        ...
        foreach ($loggers as $assignee) {
            array_push($labels, get_user_full_name($assignee['staff_id']));
    ...
    public function get_timesheeets($task_id)
    {
        ...
        return $this->db->query("SELECT id,note,start_time,end_time,task_id,user_id, CONCAT(firstname, ' ', lastname) as full_name,
        end_time - start_time time_spent FROM " . db_prefix() . 'taskstimers JOIN ' . db_prefix() . 'admin ON ' . db_prefix() . 'admin.admin_id=' . db_prefix() . "taskstimers.user_id WHERE task_id = '$task_id' ORDER BY start_time DESC")->result_array();
    }
    ...
    public function get_staff_members_that_can_access_task($task_id)
    {
        ...
        return $this->db->query('SELECT * FROM ' . db_prefix() . 'admin
            WHERE (
                    admin_id IN (SELECT user_id FROM ' . db_prefix() . "task_assigned WHERE taskid='.$task_id.')
                    OR admin_id IN (SELECT user_id FROM " . db_prefix() . 'task_followers WHERE taskid='.$task_id.')
                )
