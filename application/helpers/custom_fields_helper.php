    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            ...
            if ($field['type'] == 'input' || $field['type'] == 'number') {
                ...
                $fields_html .= '<select ' . $select_attrs . ' name="' . $select_name . '" class="' . ($items_add_edit_preview == false ? 'select-placeholder ': '') . 'selectpicker form-control' . ($field['type'] == 'multiselect' ? ' custom-field-multi-select' : '') . '" data-width="100%" data-none-selected-text="' . _l('Nichts ausgewählt') . '"  data-live-search="true">';
