...
function add_calendar_book_assets($group = 'admin', $tryGcal = true)
{

    $locale = $GLOBALS['locale'];
    $CI     = &get_instance();

    $CI->app_scripts->add('full-calendar-book-js', 'assets/plugins/bootstrap-year-calendar/bootstrap-year-calendar.js', $group);

    if ($locale != 'en' && file_exists(FCPATH . 'assets/plugins/bootstrap-year-calendar/bootstrap-year-calendar.' . $locale . '.js')) {
        $CI->app_scripts->add('full-calendar-book-lang-js', 'assets/plugins/bootstrap-year-calendar/bootstrap-year-calendar.' . $locale . '.js', $group);
    }

    $CI->app_css->add('bootstrap-year-calendar-css', 'assets/plugins/bootstrap-year-calendar/bootstrap-year-calendar.css', $group);
}
...
function add_datatables_js_assets($group = 'admin')
{
    ...
    get_instance()->app_scripts->add('datatables-fixedHeader-js', 'assets/plugins/datatables/dataTables.fixedHeader.min.js', $group);
    get_instance()->app_scripts->add('filterDropDown-js', 'assets/js/filterDropDown.min.js', $group);
}
