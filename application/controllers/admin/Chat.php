...
class chat extends MY_Controller
{
	...
	public function index()
	{
		...
		$this->load->view('admin/chat/chat_full_view', ['unreadMessages' => $result]);
	}
	...
	public function initiateChat()
	{
		if ($this->input->post()) {
			if ($this->input->post('typing') == 'false') {
				$imageData['sender_image']   = $this->chat_model->getUserImage(get_user_id());
				...
				$this->pusher->trigger('presence-mychanel-'.get_user_company_id(), 'send-event', array(
				...
				$this->pusher->trigger(
					'presence-mychanel-'.get_user_company_id(),
					'notify-event',
					array(
						'from' => $this->input->post('from'),
	...
	public function chatGroups()
	{
		...
		$this->load->view('admin/chat/includes/groups_modal', $data);
	}
